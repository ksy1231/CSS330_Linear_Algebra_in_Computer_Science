...
def add(A, B):
    ...
    for r in A.D[0]:
        for c in A.D[1]:
            k = (r, c)
            if k in A.f or k in B.f:   # maintain the sparsity
                retm[k] = A[k] + B[k]
    return retm
    """
    return Mat(A.D, {(r, c): A[r, c] + B[r, c] for r in A.D[0] for c in A.D[1] if (r, c) in A.f or (r, c) in B.f})
...
def scalar_mul(M, x):
    ...
    for r in M.D[0]:
        for c in M.D[1]:
            k = (r, c)
            if k in M.f:   # maintain the sparsity
                retm[k] = x * M[k]
    return retm
    """
    return Mat(M.D, {(r, c): x * M[(r, c)] for r in M.D[0] for c in M.D[1] if (r, c) in M.f})
...
def transpose(M):
    ...
    """
    pass
    return Mat((M.D[1], M.D[0]), {(c, r): M[r, c] for r in M.D[0] for c in M.D[1] if (r, c) in M.f})


def local_mat_to_col_vec(M):
    return {col: Vec(M.D[0], {row: M[row, col] for row in M.D[0] if (row, col) in M.f}) for col in M.D[1]}


def local_mat_to_row_vec(M):
    return {row: Vec(M.D[1], {col: M[row, col] for col in M.D[1] if (row, col) in M.f}) for row in M.D[0]}
...
def vector_matrix_mul(v, M):
    ...
    assert M.D[0] == v.D
    pass
    colm = local_mat_to_col_vec(M)
    return Vec(M.D[1], {ckey: v*colm[ckey] for ckey in M.D[1]})
...
def matrix_vector_mul(M, v):
    ...
    assert M.D[1] == v.D
    pass
    rowm = local_mat_to_row_vec(M)
    return Vec(M.D[0], {rkey: v*rowm[rkey] for rkey in M.D[0]})
...
def matrix_matrix_mul(A, B):
    ...
    #    ColVectorList = matrix_vector_mul for each of the columns in B
    #    Turn ColVectorList into a matrix
    colm = local_mat_to_col_vec(B)
    result_col_m = dict()
    for elm in colm:
        d = {elm: A*colm[elm]}
        result_col_m[elm] = d[elm]
    # turn column dict to matrix
    f = dict()
    for r in A.D[0]:
        for c in B.D[1]:
            if c in result_col_m:
                v = result_col_m[c]
                if r in v.f:
                    f[(r, c)] = result_col_m[c][r]
    d = (A.D[0], B.D[1])
    return Mat(d, f)
